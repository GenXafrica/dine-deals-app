...
import { Save, User, Mail, Eye, EyeOff, Send } from 'lucide-react';
...
interface AdminEmailSettingsCustomerProps {
  template: EmailTemplate | null;
  onUpdate: (template: EmailTemplate) => void;
  onRefresh: () => void;
}

export default function AdminEmailSettingsCustomer({
  template,
  onUpdate,
  onRefresh
}: AdminEmailSettingsCustomerProps) {
  ...
  const handleSave = async () => {
    try {
      ...
      const templateData = {
        // IMPORTANT: match Supabase row key
  ...
  const handleResend = async () => {
    ...
    const response = await adminFunctionInvoke('send-customer-welcome-email', {
      email: sendTo,
